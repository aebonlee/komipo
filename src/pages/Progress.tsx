...
const Progress = (): ReactElement => {
  // Static demo data (will be replaced with Supabase data)
  const totalStamps = 0;
                ...
                <div key={level.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ fontWeight: 600 }}>{level.title}</span>
                      <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>0 / {level.questionCount} 도장</span>
                    </div>
                    <div className="progress-bar-wrapper">
                      <div className="progress-bar-fill" style={{ width: '0%', background: level.color }} />
                    </div>
                  </div>
                </div>
              ))}
